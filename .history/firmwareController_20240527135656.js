        ...
        if(err) {
          ...
          const res = await pool.query( "INSERT INTO firmware_handler ( id, creationdate, description, firmwaredata, version ) VALUES ($1, $2,$3, $4, $5) RETURNING *",
            ...
            if (error) {console.log(error) }
    	      ...
    	      console.log(`resultsFirm added with ID: ${resultsFirm}`);
      ...
      if(err) {
        ...
        const resSelect = await pool.query('SELECT * FROM firmware_handler', async (error, resultsFirmwareList) => {
          ...
          resultsFirmwareList.rows.forEach((firm)=>{
            firmwareList.push({creationDate: firm.firmwareObject.creationdate, firmwareData:firm.firmwareObject.firmwaredata, description: firm.firmwareObject.description, date: firm.firmwareObject.date,id:firm.firmwareObject.id});
          })
          // firmwareList
          console.log('Result of getFirmwaresList : !  ',resultsFirmwareList.rows)
          return res.status(200).json({
            response: {
              result:'success',
              message:''
