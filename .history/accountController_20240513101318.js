            ...
            if(dataBodyOfRequest.owner !== undefined){
              const resSelect = await pool.query('SELECT * FROM account_handler WHERE id = $1',[dataBodyOfRequest.owner], (error, results) => {
                ...
                if(results.rowCount === 0){
                  ...
                  if(dataBodyOfRequest.role === 'users' || dataBodyOfRequest.role === undefined){
      ...
      jwt.verify(token, 'secret', { expiresIn: '30d' }, async function(err, decoded) {
        if(err) {
          ...
          if(req.headers.email !== undefined){
            ...
            const resSelect = await pool.query('SELECT * FROM account_handler WHERE email = $1',[userEmail], async (error, results) => {
              if (error) {
                console.log(error)
              }
              if(results.rowCount === 0){
              ...
              if(results.rowCount === 1){
                ...
                if(results.rows[0].role === 'admin'){
                  const resSelect = await pool.query('SELECT * FROM account_handler WHERE IN ($1,$2);',['admin','owner'], (error, results) => {
                    if (error) {
                      console.log(error)
                    }
                   console.log('RESULTS AFTER SELECT OWNER OR ADMIN : ',results.rows)
                   res.status(200).json({
                    response: {
                      result: 'success',
                      message: ''
          ...
          if(req.headers.id !== undefined){
            ...
            const resSelect = await pool.query('SELECT * FROM account_handler WHERE id = $1',[userId], (error, results) => {
              if (error) {
                console.log(error)
              }
              if(results.rowCount === 0){
