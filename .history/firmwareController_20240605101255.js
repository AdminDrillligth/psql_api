      ...
      if(err) {
        ...
        const lastpublicchangecount = await pool.query('SELECT publicfirmwareid FROM global_handler', async (error, publicFirmwareId) => {
          ...
          if( publicFirmwareId !== undefined){
            const resSelectGlobal = await pool.query('SELECT * FROM firmware_handler WHERE id = $1',[publicFirmwareId.rows[0]], (error, resSelectGlobal) => {
              if (error) {
                console.log(error)
              }
              ...
              return res.status(200).json({
                ...
                globalFirmware:resSelectGlobal.rows[0]
              });
            })
