              ...
              const resSelect = await pool.query('SELECT * FROM account_handler WHERE id = $1',[dataBodyOfRequest.owner], (error, results) => {
                ...
                if(results.rowCount === 0){
                  ...
                  if(dataBodyOfRequest.role === 'user' || dataBodyOfRequest.role === undefined){
                    ...
                    const res = pool.query('INSERT INTO account_handler (users) VALUES ($1) WHERE id = $2', [JSON.stringify([{id:id}]) , dataBodyOfRequest.owner] ,
                    (error, results) => {
                      if (error) {
                        throw error
                      }
                    ...
                    // [JSON.stringify({id:id}), dataBodyOfRequest.owner],
                    // (error, results) => {
                    //   if (error) {
                    //     throw error
                    //   }
            ...
            const resSelect = await pool.query('SELECT * FROM account_handler WHERE id = $1',[id], async (error, results) => {
              ...
              if(results.rowCount === 1){
                ...
                if(results.rows[0].role === 'admin'){
                   ...
                   console.log('RESULTS AFTER SELECT OWNER OR ADMIN : ',results.rows[0])
                   res.status(200).json({
                    response: {
                      result: 'success',
                      message: ''
