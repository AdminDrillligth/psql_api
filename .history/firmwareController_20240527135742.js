      ...
      if(err) {
        ...
        const resSelect = await pool.query('SELECT * FROM firmware_handler', async (error, resultsFirmwareList) => {
          ...
          resultsFirmwareList.rows.forEach((firm)=>{
            firmwareList.push({creationDate: firm.creationdate, firmwareData:firm.firmwareObject.firmwaredata, description: firm.firmwareObject.description, date: firm.firmwareObject.date,id:firm.firmwareObject.id});
          })
          // firmwareList
