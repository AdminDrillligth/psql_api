                  ...
                  //  console.log('RESULTS AFTER SELECT ADMIN : ', accountSelected.users, accountSelected.users.length)
                   if(accountSelected.users.length > 0){
                    ...
                    accountSelected.users.forEach(async user => {
                      ...
                      const selectUser = await pool.query('SELECT * FROM account_handler WHERE id = $1',[user.id], async (error, results) => {
                        if (error) {
                          console.log(error)
                        }
                        validateUser = false;
                        console.log(results.rows.passwordhash,results.rows.passwordHash)
                        if(results.rows.passwordhash !== undefined){
                          validateUser = true;
                        }
                        console.log('RESULT USERS OF GET DETAILS :  ! ', results.rows)
                        accountUsers.push({validate:validateUser, id: results.rows.id, email:results.rows.email, fullName:results.rows.fullName,familyName:results.rows.familyName, firstName: results.rows.firstName });
