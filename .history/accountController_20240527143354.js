            ...
            const resSelect = await pool.query('SELECT * FROM account_handler WHERE id = $1',[id], async (error, results) => {
              ...
              if(results.rowCount === 1){
                ...
                if(results.rows[0].role === 'admin'){
                  ...
                  //  accountDetails = {avartarUrl:,date:,dateIso:,econes:,email:,familyName:,firstName:,fullName:,id:,licensed:, owner:, passwordHash:,personalInfo:, privateExercisesChangeCount:, privateFirmwareId:,privateOnly:,privileges:,staff:,trainings:,uid:,update:,updateIso:,users:,videos:,warning:}
                   res.status(200).json({
                    response: {
                      result: 'success',
                      message: ''
