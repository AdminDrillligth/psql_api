                   ...
                   if(accountSelected.users.length > 0){
                    ...
                    accountSelected.users.forEach(async user => {
                      ...
                      const selectUser = await pool.query('SELECT * FROM account_handler WHERE id = $1',[user.id], async (error, resultsUser) => {
                        ...
                        validateUser = false;
                        console.log('RESULT USERS OF GET DETAILS :  ! ', resultsUser.rows)
                        console.log(resultsUser.rows.passwordhash,resultsUser.rows.passwordHash)
                        if(resultsUser.rows.passwordhash !== undefined){
                          validateUser = true;
                        }
                        console.log('RESULT USERS OF GET DETAILS :  ! ', resultsUser.rows)
                        accountUsers.push({validate:validateUser, id: resultsUser.rows.id, email:resultsUser.rows.email, fullName:resultsUser.rows.fullName,familyName:resultsUser.rows.familyName, firstName: resultsUser.rows.firstName });
